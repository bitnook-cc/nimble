...
// Extract text excerpt from MDX content for search indexing
...
export default defineConfig({
  ...
  item?: {
    ...
    // Build search indices with Fuse.js
    console.log('[VELITE] Building search indices...')

    const searchDocuments: any[] = []

    // Collect all documents for searching
    Object.entries(data).forEach(([collectionName, collection]) => {
      if (Array.isArray(collection) && collection.length > 0) {
        collection.forEach((item: any) => {
          searchDocuments.push({
            ...
            tags: item.tags || [],
            contentExcerpt: extractExcerpt(item.content || ''),
            access: item.access || [],
            permalink: item.permalink,
    ...
    // Create Fuse index with optimized keys
    const fuseIndex = Fuse.createIndex(
      ['title', 'description', 'category', 'tags', 'contentExcerpt'],
      searchDocuments
    )

    // Save search data and index
    const searchData = {
    ...
    // Generate TypeScript definitions for search
    const searchTypes = `// This file is generated by Velite
...
export interface SearchIndex {
  ...
  index: any // Fuse.js index data
}

declare const searchIndex: SearchIndex
export default searchIndex`

    writeFileSync(
      join(outputDir, 'search-index.d.ts'),
      searchTypes,
      'utf-8'
    )

    console.log(`[VELITE] ✅ Built search index: ${searchDocuments.length} documents`)
  }
})