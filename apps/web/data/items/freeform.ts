import { RepositoryFreeformItem } from "@/lib/types/item-repository";

export const MUNDANE_FREEFORM: RepositoryFreeformItem[] = [
  {
    id: "backpack",
    name: "Backpack",
    type: "freeform",
    size: 2,
    description: "A sturdy leather pack with multiple compartments for carrying gear.",
    category: "mundane",
  },
  {
    id: "bedroll",
    name: "Bedroll",
    type: "freeform",
    size: 2,
    description: "A thick blanket and sleeping pad for resting outdoors.",
    category: "mundane",
  },
  {
    id: "thieves-tools",
    name: "Thieves' Tools",
    type: "freeform",
    size: 1,
    description:
      "A set of lock picks, skeleton keys, and other tools for bypassing locks and traps.",
    category: "mundane",
  },
  {
    id: "grappling-hook",
    name: "Grappling Hook",
    type: "freeform",
    size: 2,
    description: "A three-pronged iron hook attached to a length of rope for climbing.",
    category: "mundane",
  },
  {
    id: "lantern",
    name: "Lantern",
    type: "freeform",
    size: 1,
    description: "A metal lantern with glass panes that burns oil to provide steady light.",
    category: "mundane",
  },
  {
    id: "spellbook",
    name: "Spellbook",
    type: "freeform",
    size: 2,
    description:
      "A leather-bound tome filled with blank pages for recording spells and magical formulae.",
    category: "mundane",
  },
  {
    id: "healers-kit",
    name: "Healer's Kit",
    type: "freeform",
    size: 1,
    description: "Bandages, herbs, and medical supplies for treating wounds in the field.",
    category: "mundane",
  },
  {
    id: "tinderbox",
    name: "Tinderbox",
    type: "freeform",
    size: 1,
    description: "Flint, steel, and tinder for starting fires.",
    category: "mundane",
  },
  {
    id: "chalk",
    name: "Chalk",
    type: "freeform",
    size: 0,
    description: "White chalk for marking surfaces and leaving messages.",
    category: "mundane",
  },
  {
    id: "javelins-4",
    name: "Javelins (4)",
    type: "freeform",
    size: 2,
    description: "Four light throwing spears used for hunting or combat.",
    category: "mundane",
  },
  {
    id: "hunting-trap",
    name: "Hunting Trap",
    type: "freeform",
    size: 2,
    description: "A mechanical trap with sharp teeth for catching game or intruders.",
    category: "mundane",
  },
  {
    id: "soap",
    name: "Soap",
    type: "freeform",
    size: 0,
    description: "A bar of soap for cleaning and hygiene.",
    category: "mundane",
  },
  {
    id: "manacles",
    name: "Manacles",
    type: "freeform",
    size: 1,
    description: "Iron shackles for restraining prisoners.",
    category: "mundane",
  },
  {
    id: "shovel",
    name: "Shovel",
    type: "freeform",
    size: 2,
    description: "A sturdy digging tool for excavation.",
    category: "mundane",
  },
  {
    id: "bell",
    name: "Bell",
    type: "freeform",
    size: 0,
    description: "A small brass bell that rings clearly when shaken.",
    category: "mundane",
  },
  {
    id: "instrument",
    name: "Musical Instrument",
    type: "freeform",
    size: 1,
    description: "A flute, lute, or other instrument for making music.",
    category: "mundane",
  },
  {
    id: "mirror",
    name: "Small Mirror",
    type: "freeform",
    size: 0,
    description: "A polished metal mirror for signaling or grooming.",
    category: "mundane",
  },
  {
    id: "strange-plant",
    name: "Strange Plant",
    type: "freeform",
    size: 1,
    description: "An unusual potted plant with unknown properties.",
    category: "mundane",
  },
  {
    id: "sandals",
    name: "Sandals",
    type: "freeform",
    size: 0,
    description: "Simple leather sandals for warm weather travel.",
    category: "mundane",
  },
];

export const MAGICAL_FREEFORM: RepositoryFreeformItem[] = [
  {
    id: "bag-of-holding",
    name: "Bag of Holding",
    type: "freeform",
    size: 1,
    description:
      "A magical satchel that contains an extradimensional space, holding far more than its size suggests.",
    category: "magical",
    rarity: "uncommon",
  },
  {
    id: "crystal-ball",
    name: "Crystal Ball",
    type: "freeform",
    size: 2,
    description:
      "A sphere of pure crystal that allows the user to scry distant locations and people.",
    category: "magical",
    rarity: "very-rare",
  },
  {
    id: "wand-of-magic-missiles",
    name: "Wand of Magic Missiles",
    type: "freeform",
    size: 1,
    description: "A slender wand that fires unerring bolts of magical force at enemies.",
    category: "magical",
    rarity: "uncommon",
  },
  {
    id: "boots-of-speed",
    name: "Boots of Speed",
    type: "freeform",
    size: 1,
    description: "Enchanted boots that allow the wearer to move with supernatural swiftness.",
    category: "magical",
    rarity: "rare",
  },
  {
    id: "ring-of-power",
    name: "Ring of Power",
    type: "freeform",
    size: 0,
    description:
      "A golden ring inscribed with ancient runes that enhances the magical abilities of its wearer.",
    category: "magical",
    rarity: "legendary",
  },
  {
    id: "cloak-of-elvenkind",
    name: "Cloak of Elvenkind",
    type: "freeform",
    size: 1,
    description:
      "A gray cloak woven by elves that helps its wearer blend into natural surroundings.",
    category: "magical",
    rarity: "uncommon",
  },
];
